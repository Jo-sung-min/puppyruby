...
if ([string]::IsNullOrWhiteSpace($env:DB_URL) -or [string]::IsNullOrWhiteSpace($env:DB_USERNAME)) {
    throw 'DB_URL and DB_USERNAME are required. Create backend/.env from .env.example and enter the PostgreSQL connection settings.'
}
