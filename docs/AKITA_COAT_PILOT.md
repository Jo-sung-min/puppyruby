...
## 승인한 아키타 외형 적용 (2026-09-22)

사용자가 승인한 수정본을 아키타의 실제 웹 표시·옷장·관리자·Windows 외형 응답과 새 설치파일의 오프라인 기본 이미지에 적용했다. 관리자 **아키타 적용본 · 깨끗한 얼굴과 네 발**과 기존 도트 스타일 미리보기에서 확인한다. 다른 29견종은 변경하지 않았다. 에셋 102개(PNG 및 Aseprite 원본)를 S3에 추가하고 `https://cdn.puppyruby.com/site-packs/akita-coat`의 파일 해시를 검증했다. 운영 웹 코드는 사용자가 재배포해야 하며, 사이트 다운로드 릴리스와 설치된 앱은 자동 교체하지 않았다.
...
- 관리자에서 30가지 눈, 5가지 털색, 눈 숨기기, 프레임별 정지, 밝은색·어두운색·체크무늬 배경으로 검수할 수 있다. 머리 위치가 바뀐 옆모습·걷기·인사의 슬롯 좌표도 조정했다. 액세서리 작업실에서 프레임별 추가 미세 조정이 가능하다.
- 웹은 감은 눈도 별도 PNG로 합성한다. 기존 Windows 계약을 유지하기 위해 전송·내장용 복사본에만 감은 눈을 미리 합성하고 그 픽셀은 염색에서 제외한다. 편집용 원본과 웹 몸통은 눈 없는 상태를 유지한다. 구형 클라이언트의 다섯 장면 프레임 수(걷기 4, 나머지 1)를 보존한다.
...
스타일 확정 전 비용을 줄이기 위해 **아키타 한 종만** 적용한다. 승인한 수정본의 16동작 × 4프레임을 사용한다. 다른 29견종은 기존 렌더링을 유지한다.
...
생성 명령은 `.\desktop\build.ps1 -SkipPublish`다. 새 아키타를 내장하기 위해 Export를 건너뛰지 않는다. 설치된 프로그램과 웹 다운로드 릴리스는 아직 교체하지 않았다. 로컬 연동 시험 시 프로그램이 해당 개발 서버에 연결되어 있어야 한다.

## 운영에 적용할 때

등록된 `akita-art-release.json`의 CDN 주소를 기본 사용하므로 별도 환경변수 없이 배포할 수 있다. 새 바이너리는 Git에 올리지 않는다. 염색용 픽셀 읽기는 `/api/akita-coat/{sha256}.png`의 같은 출처 프록시를 사용한다. 프록시는 허용목록에 있는 파일만 공개 CDN에서 읽고 내용 해시·용량을 검증하며 리다이렉트를 거부한다. CDN의 브라우저 CORS 설정에 의존하지 않는다. 원본 몸통·눈 표시 및 Windows 다운로드는 CDN URL을 직접 사용한다.

추후 승인본을 바꾸면 `scripts/publish-akita-art.ps1`로 정해진 S3 경로에 해시 파일을 추가한다. 모든 파일의 CDN 검증이 끝나야 릴리스 메타데이터가 바뀐다. 선택적으로 표시용 기본 URL을 바꾸려면 다음 설정을 쓸 수 있지만 염색 프록시는 검증된 릴리스 주소를 계속 사용한다.

```dotenv
NEXT_PUBLIC_AKITA_COAT_BASE_URL=https://cdn.example.com/akita-coat
```

내용 기반 파일명은 덮어쓰지 않는다. 운영 관리자에서 위치 저장은 아직 지원하지 않으며, 로컬에서 저장한 메타데이터를 코드와 함께 배포한다. 배포 전 `node scripts/verify-akita-promotion.cjs`로 내장 이미지, 구버전 호환성과 웹·Windows 색상/눈 합성 일치를 검사한다.
