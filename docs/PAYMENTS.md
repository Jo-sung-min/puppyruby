...
루트 `.env.example` 또는 `backend/.env.example`의 빈 항목에 운영자가 발급받은 값을 **로컬 설정 파일/배포 환경 변수**로 지정합니다. 키를 소스 코드나 문서, 프런트엔드 환경 변수에 넣지 마세요.
...
| `PUBLIC_SITE_URL` | 브라우저에서 접속하는 루트 origin. HTTPS 또는 localhost/127.0.0.1/[::1]의 HTTP만 허용합니다. 경로·쿼리·사용자 정보는 허용하지 않습니다. | `http://127.0.0.1:3001` |
...
Windows 실행기는 `backend/.env.local`에서 위 세 토스 설정을 읽습니다. 이 파일은 저장소에 올리지 않습니다. Docker에서는 루트 `.env`와 Compose의 백엔드 환경 전달 설정을 사용합니다. 서버가 읽는 토스 API 주소는 `https://api.tosspayments.com/v1/payments/`로 고정되어 있고 환경 변수로 외부 주소를 지정할 수 없습니다.
