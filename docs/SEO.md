...
대표 주소를 비우면 프런트 서버의 `PUBLIC_SITE_URL`을 사용합니다. 공개 HTTPS 주소가 없거나 로컬 주소이면 canonical과 사이트맵 URL을 생성하지 않습니다. 프런트의 `API_URL`은 AWS Elastic Beanstalk의 공개 HTTPS origin으로 설정하며 Next.js 프록시가 `/api/v1`을 자동으로 붙입니다. 관리자는 이 화면에서 실제 도메인 연결이나 로그인 리다이렉트 설정을 변경하는 것이 아닙니다.
