...
$spriteBuild = Join-Path (Split-Path $spriteRoot -Parent) 'local-assets/desktop/build'
        ...
        static int Main(string[] args) {
            try {
                ...
                using (var library = new LinkedSpriteLibrary()) {
                    ...
                    string[] web = DesktopBreedCatalog.Ids;
                    Check(web.Length == 30 && web[5] == "beagle" && web[29] == "chowchow", "Catalog retains legacy indices and all new breeds");
                    ...
                    Check(Object.ReferenceEquals(library.Get(web.Length, "original", "original", "none", "idle", 0, 0), library.Get("pomeranian", "idle", 0, 0, "original", "original", "none")), "Upper-bound numeric breed uses web default");
                ...
                Console.WriteLine("PASS: " + samples.Count + " web-reference sprites, " + checkedPixels + " native pixel comparisons, " + DesktopBreedCatalog.Ids.Length + " web breed mappings, fallback normalization and bounded LRU/disposal.");
                return 0;
...
Get-ChildItem -LiteralPath $spriteAssets -File | Where-Object { $_.Name -eq 'breed-catalog.json' -or $_.Name -eq 'linked-sprites.json' -or $_.Name -match '^linked-[a-f0-9]{64}\.png$' } | Sort-Object Name | ForEach-Object {
    $spriteArguments += '/resource:"' + $_.FullName + '",' + $_.Name
}
$spriteArguments += '"' + (Join-Path $spriteRoot 'LinkedSpriteLibrary.cs') + '"'
$spriteArguments += '"' + (Join-Path $spriteRoot 'DesktopBreedCatalog.cs') + '"'
