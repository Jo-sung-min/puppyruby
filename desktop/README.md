# PuppyRuby Desktop · Windows 픽셀 강아지

0.10.2는 강아지와 트레이의 우클릭 메뉴를 같은 어두운 스타일로 표시합니다.
**물어보기 · 배운 명령**, **숨기기**, **업데이트 확인**, **종료**는 바로 사용할 수 있고,
나머지는 **돌봄**, **움직임**, **강아지**, **웹 연결**, **설정**에서 찾을 수 있습니다.
긴 연결·외형 설명은 관련 창과 툴팁에서 확인하며, Windows 고대비 모드에서는 기본 메뉴 색상을 사용합니다.

새 Windows 앱은 웹과 같은 **30견종·열여섯 동작**을 사용합니다. 배를 보이며
눕기, 타이핑, 쓰다듬기, 기지개와 네 방향 걷기까지 원본의 네 프레임으로
움직입니다. 설치 파일에 모든 동작이 들어 있어 연결 전이나 오프라인에도
볼 수 있습니다. 웹과 연결하면 선택한 눈과 액세서리도 함께 적용됩니다.
이전 앱의 다섯 장면 연결은 계속 유지되며, 새 동작은 최신 앱으로 업데이트하면
사용할 수 있습니다.

최신 앱은 실행할 때와 이후 정기적으로 새 버전을 확인합니다. 새 버전이 있으면 **강아지 위에 업데이트 버튼**이 나타나며, 누르면 다운로드 진행률을 보여 준 뒤 설치하고 다시 실행합니다. 우클릭 메뉴의 **업데이트 확인**으로 직접 확인할 수도 있습니다. 기존 강아지 기록과 웹 연결 정보는 유지됩니다.

업데이트 버튼이 없는 이전 실행파일은 강아지를 종료하고 사이트의 최신 설치파일을 **한 번 설치**해 주세요. 이후부터 새 버전은 강아지 위 버튼으로 받을 수 있습니다. 배포 절차는 [업데이트 배포 안내](UPDATES.md)에 있습니다.

**1.2초 안에 왼쪽 클릭 5번**을 감지하면 강아지가 배를 보이며 뒤집어집니다.
...
`https://www.puppyruby.com`이며 연결·상태 동기화·돌봄·업데이트 확인에 사용합니다.
이전에 저장한 `https://puppyruby.com` 연결도 앱을 시작하면 같은 주소로 전환합니다.
이미지와 설치파일은 검증된 CDN 주소로 받으며, 로컬 개발용 주소는 연결 창에서 직접 지정할 수 있습니다.

사용자 참고 이미지에 맞춰 다시 그린 30견종의 열여섯 동작을 웹과 앱에서 함께 사용합니다. 견종별 원본 얼굴에 맞춰 눈과 액세서리를 합성하며, 감은 눈과 발바닥이 보이는 자세는 원본 그림을 유지합니다. 실행 중인 이전 버전은 종료한 뒤 새 실행파일을 열어야 새 그림이 적용됩니다.
...
- **우클릭 → 움직임 → 마우스 따라가기**: 커서 옆으로 걷기 시작합니다.
- **우클릭 → 움직임 → 여기에 멈추기**: 현재 위치에 즉시 멈추며 다시 실행해도 유지합니다. 키보드·마우스 반응과 웹 연결은 계속됩니다.
- **직접 드래그**: 놓은 자리에 머뭅니다. 다시 걷게 하려면 마우스 따라가기를 누릅니다.
- **우클릭 → 설정 → 입력 반응 일시정지**: 자동 이동과 입력 감지를 함께 멈춥니다. 질문 창에서 직접 요청한 훈련은 볼 수 있습니다.
...
2. 실행파일의 강아지 또는 트레이 아이콘을 우클릭하고 **웹 연결 → 웹 강아지와 연결**을 엽니다. 이미 연결돼 있으면 **연결 관리**를 엽니다.
...
**우클릭 → 물어보기 · 배운 명령**에서 앱을 선택하고 `엑셀 붙여넣기 단축키 알려줘`처럼 입력하면 `Ctrl + V다 멍!` 하고 3.5초 동안 말풍선으로 답합니다. 질문 창에는 전체 설명과 공식 출처가 남습니다. 창을 닫으면 질문과 답변은 보관하지 않습니다. 엑셀·한글에서 단축키를 실제로 실행하는 기능은 아니며, 사용법을 알려주는 도우미입니다.
...
- 웹 연결 중에는 지정한 사이트의 연결·강아지 상태·명시적으로 요청한 게임 동작을 통신합니다. 별도로 새 버전 안내를 확인하고 업데이트 버튼을 누르면 설치파일을 받습니다. 실명·사진·친구·산책방·채팅은 가져오지 않으며 전역 입력은 네트워크 요청을 만들지 않습니다. 기존 `NativeInput.cs`는 변경하지 않았습니다.
- 현재 빌드는 코드 서명이 없는 개발 빌드이며, 현재 사용자용 설치 프로그램과 사용자가 버튼을 눌러 진행하는 업데이트를 제공합니다.
...
`desktop/export-ruby-default.cjs`가 등록된 루비 도트 PNG의 해시와 원본 크기를 확인하고, 30견종의 열여섯 동작·동작당 네 프레임을 그대로 내보냅니다. 연결 강아지의 눈과 액세서리는 웹과 같은 리소스로 합성합니다. Windows 기본 C# 컴파일러로 원본 몸체·공용 눈과 `shared/commands.json`을 내장한 단일 실행파일을 만듭니다. 캐릭터나 명령 사전이 변경되면 다시 빌드하세요.
...
바탕화면용 그림은 원본 PNG의 투명도와 픽셀을 유지합니다. 새 앱은 모든 동작의 네 프레임을 사용하고, 이전 앱용 다섯 장면 이미지도 함께 보관합니다.

다운로드 게시 도구의 로컬 검사는 `scripts/publish-site-downloads.ps1 -TestOnly`, 게시 계획은 `-Action Plan`으로 실행합니다. 두 명령은 업로드하지 않습니다. 오프라인 Gradle 의존성을 읽을 수 없으면 이미 빌드한 최신 백엔드 Boot JAR에서 정확한 런타임 라이브러리만 추출합니다. 선택한 JAR의 해시별로 보관하고 원본·추출 경로를 검증하며, 사용한 경로는 `backend/build/site-download-publisher/runtime-source.txt`에 기록합니다. 실제 공개 다운로드 목록은 실행파일 2개와 체크섬 2개로 제한합니다.
