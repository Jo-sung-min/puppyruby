    ...
    internal sealed class DesktopAppearanceFrames : IDisposable
    {
        ...
        internal DesktopAppearanceFrames(DesktopAppearance descriptor, Dictionary<string, Bitmap> sheets)
        {
            try
            {
                ...
                Key = descriptor.key; StyleId = descriptor.styleId; BreedId = descriptor.breedId;
                ...
                foreach (string name in new[] { "idle", "side", "walk", "happy", "sleep" })
                {
                    ...
                    if (!descriptor.scenes.TryGetValue(name, out spec) || spec == null || !sheets.TryGetValue(name, out sheet) || sheet == null || spec.frames < 1 || spec.frames > 8 || spec.frameMs < 16 || spec.frameMs > 10000 || sheet.Width != Width * spec.frames || sheet.Height != Height)
                        throw new InvalidDataException("강아지 장면의 프레임 크기가 맞지 않아요.");
                    string fingerprint = String.IsNullOrEmpty(spec.sha256) ? name : spec.sha256 + ":" + spec.frames;
                    Scene scene;
                    if (!unique.TryGetValue(fingerprint, out scene))
                    {
                        scene = new Scene { Normal = new DesktopAppearanceFrame[spec.frames], Flipped = new DesktopAppearanceFrame[spec.frames], FrameMs = spec.frameMs };
