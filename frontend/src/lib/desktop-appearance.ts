...
export function desktopAppearance(config: AppearanceConfig, breed: number, origin: string): DesktopAppearance | null {
  ...
  const identity = { version: 1 as const, styleId, styleName, breedId, width, height, scenes };
...
export async function attachDesktopAppearance(data: unknown, action: string, origin: string, backendBase: string, fetcher: typeof fetch = fetch): Promise<unknown> {
  ...
  try {
    ...
    appearance = desktopAppearance(config, state.puppy.breed, origin);
  } catch (error) {
