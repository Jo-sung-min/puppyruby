...
import { art16FrameIndex, art16SceneAsset, art16SceneForMood, art16Scenes, art16SceneStyleId, isArt16SceneId, type Art16SceneId } from "../lib/art16-scene-styles";
import { art16GazeOffset, art16ReactionAnchors } from "../lib/art16-reactions";
import { dogBreedById, type PixelBreed } from "../lib/dog-breeds";
import { Art16DogAccessories } from "./art16-dog-accessories";
import type { PixelMood } from "./pixel-dog";
import styles from "./art16-scene-pixel-dog.module.css";

type Props = { breed: PixelBreed; mood: PixelMood; scene?: Art16SceneId; paused?: boolean; frame?: number; look?: number; lookY?: number;
...
export function Art16ScenePixelDog({ breed, mood, scene, paused = false, frame = 0, look = 0, lookY = 0, accessory = "none", className = "", decorative = false, groundShadow = true }: Props) {
  const id = `art16-${useId().replace(/[^a-zA-Z0-9_-]/g, "")}`;
  const asset = art16SceneAsset(breed);
  if (!asset) return null;
  const scenePreview = isArt16SceneId(scene);
  const reacting = !scenePreview && !paused;
  const activeScene = scenePreview ? scene : paused ? "idle" : art16SceneForMood(mood);
  ...
  const label = `${dogBreedById(breed).name} · ${art16Scenes.find(item => item.id === activeScene)?.name}`;
  const native = art16ReactionAnchors(breed, asset.width, asset.height);
  ...
  return <svg viewBox={`0 0 ${asset.width} ${asset.height}`} className={`pixel-dog dog-style-art16-scenes ${styles.dog} ${className}`} style={animationStyle}
    data-dog-style={art16SceneStyleId} data-dog-breed={breed} data-dog-scene={activeScene} data-frame-count={sheet.frames}
    ...
    <g className={styles.body}>
      <svg x="0" y="0" width={asset.width} height={asset.height} viewBox={`0 0 ${asset.width} ${asset.height}`} preserveAspectRatio="xMidYMid meet" className={styles.viewport}>
        <g key={`${breed}-${activeScene}`} className={`${styles.strip}${animated ? ` ${styles.playing}` : ""}`}>
          {accessory === "angel-wings" && Array.from({ length: sheet.frames }, (_, n) => <g key={`back-${n}`} transform={`translate(${(n - index) * asset.width} 0)`}>
            <g clipPath={`url(#${id}-frame)`}><Art16DogAccessories id={accessory} breed={breed} scene={activeScene} width={asset.width} height={asset.height} frame={n} behind /></g>
          </g>)}
          ...
          {accessory !== "none" && accessory !== "angel-wings" && Array.from({ length: sheet.frames }, (_, n) => <g key={`front-${n}`} transform={`translate(${(n - index) * asset.width} 0)`}>
            <g clipPath={`url(#${id}-frame)`}><Art16DogAccessories id={accessory} breed={breed} scene={activeScene} width={asset.width} height={asset.height} frame={n} /></g>
          </g>)}
        </g>
      </svg>
    </g>
