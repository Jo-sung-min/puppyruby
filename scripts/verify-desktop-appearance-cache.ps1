...
$executable = Join-Path $testRoot 'DesktopAppearanceCacheTest.exe'
$arguments = @('/nologo', '/target:exe', '/platform:anycpu', '/optimize+', '/utf8output', '/main:PuppyRubyDesktop.DesktopAppearanceCacheTest',
...
foreach ($source in @('DesktopAppearanceCacheTest.cs', 'DesktopAppearanceCache.cs', 'DesktopAppearanceFrames.cs', 'DesktopAccessoryRenderer.cs', 'DesktopAppearanceReactions.cs', 'DesktopBreedCatalog.cs', 'DesktopSync.cs', 'Progression.cs', 'PetState.cs')) {
...
if ($LASTEXITCODE -ne 0) { throw 'Desktop appearance integration-test compilation failed.' }
& $executable $projectRoot $testRoot
if ($LASTEXITCODE -ne 0) { throw 'Desktop appearance integration tests failed.' }
