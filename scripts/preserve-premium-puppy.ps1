param(
    [Parameter(Mandatory=$true)][ValidatePattern('^premium-(marshmallow|milkbean|honeybun|cloudpuff|biscuit|naploaf|teddycub|peachcheek|buttonpaw|rounddrop|cottonball|caramel)$')][string]$Id,
...
$destination=Join-Path $workspace ('local-assets/work/premium-puppies-v1/processed/'+$Id)
...
foreach($argument in @('--batch','--script-param',('input='+$isolated),'--script-param',('output='+$destination),'--script-param',('id='+$Id),'--script',(Join-Path $PSScriptRoot 'aseprite/preserve-premium-puppy.lua'))){$start.ArgumentList.Add($argument)}
...
public static class PremiumPixelAudit {
  ...
  public static object Compare(string originalPath,string masterPath) {
    using(var original=new Bitmap(originalPath)) using(var master=new Bitmap(masterPath)) {
      ...
      Action<int> add=i=>{if(seen[i])return;seen[i]=true;int p=i*4,min=Math.Min(a[p],Math.Min(a[p+1],a[p+2])),max=Math.Max(a[p],Math.Max(a[p+1],a[p+2]));if(a[p+3]==0||(min>=165&&max-min<=12)){erase[i]=true;queue.Enqueue(i);}};
...
$original= [PremiumPixelAudit]::Inspect($source)
$master= [PremiumPixelAudit]::Inspect((Join-Path $destination ($Id+'.png')))
$comparison=[PremiumPixelAudit]::Compare($source,(Join-Path $destination ($Id+'.png')))
...
$summary=[ordered]@{id=$Id;source=$source;sourceSha256=$sourceHash;sourcePreserved=$true;exactVisibleRgba=$true;resized=$false;quantized=$false;recolored=$false;frames=1;layers=1;colorDepth=32;width=$master.width;height=$master.height;visiblePixelSha256=$master.visiblePixelSha256;visibleColors=$master.visibleColors;transparentPixels=$master.transparentPixels;partialAlphaPixels=$master.partialAlphaPixels;opaquePixels=$master.opaquePixels;bounds=$master.bounds;backgroundCleanup='Only light neutral pixels connected to image border; min RGB 165, max channel spread 12';pixelComparison=$comparison;pngSha256=(Get-FileHash -LiteralPath (Join-Path $destination ($Id+'.png'))).Hash;asepriteSha256=(Get-FileHash -LiteralPath (Join-Path $destination ($Id+'.aseprite'))).Hash}
...
if($Publish){
    foreach($extension in @('png','aseprite')){
        ...
        $publicDirectory=Join-Path $workspace ('local-assets/site/'+$folder+'/premium-puppies-v1')
    ...
    & (Join-Path $PSScriptRoot 'update-premium-puppy-data.ps1')|Out-Null
}
