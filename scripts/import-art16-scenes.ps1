...
$destination = Join-Path $workspace ('local-assets/work/art16-scenes-v1/processed/' + $Breed)
...
if (-not $VerifyExisting) {
    ...
    if (Test-Path -LiteralPath $isolated) { throw 'A previous conversion exists. Inspect it before retrying.' }
    ...
    foreach ($argument in @('--batch','--script-param',('input='+$isolated),'--script-param',('output='+$destination),'--script-param',('id='+$Breed),'--script-param',('background='+$Background),'--script',(Join-Path $PSScriptRoot 'aseprite/import-art16-scenes.lua'))) { $start.ArgumentList.Add($argument) }
...
if (-not (Test-Path -LiteralPath $proofPath)) { throw 'Aseprite did not complete and verify all five scenes.' }
$proof = Get-Content -LiteralPath $proofPath -Raw | ConvertFrom-Json
if ($proof.breed -ne $Breed -or -not $proof.exactVisibleRgba -or $proof.resized -or $proof.quantized -or $proof.recolored -or $proof.fabricatedFrames -or $proof.frames -ne 12) { throw 'Unverified scene conversion.' }
...
if ($bytes.Length -lt 128 -or [BitConverter]::ToUInt16($bytes,4) -ne 0xA5E0 -or [BitConverter]::ToUInt16($bytes,6) -ne 12 -or [BitConverter]::ToUInt16($bytes,8) -ne $proof.width -or [BitConverter]::ToUInt16($bytes,10) -ne $proof.height -or [BitConverter]::ToUInt16($bytes,12) -ne 32) { throw 'Editable Aseprite file does not preserve the 12 full-resolution RGBA frames.' }
$hashes = [ordered]@{}
...
if ($Publish) {
    $imageDirectory = Join-Path $workspace ('local-assets/site/images/art16-scenes-v1/'+$Breed)
    $downloadDirectory = Join-Path $workspace 'local-assets/site/downloads/art16-scenes-v1'
...
[ordered]@{breed=$Breed;scenes=5;frames=12;width=$proof.width;height=$proof.height;sourcePreserved=$true;exactVisibleRgba=$true;published=[bool]$Publish} | ConvertTo-Json -Compress
