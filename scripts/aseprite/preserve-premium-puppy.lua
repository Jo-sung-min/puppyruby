...
-- Generated sources can contain a baked light-gray checker. Remove only neutral
-- light pixels connected to the canvas border; enclosed artwork is untouched.
...
local function consider(x,y)
  ...
  if a==0 or (math.min(r,g,b)>=165 and math.max(r,g,b)-math.min(r,g,b)<=12) then
    queue[#queue+1]=index
  end
end
