...
const { dogStyles, styleBreedIds, styleBreeds, defaultAppearance, resolveDogStyle, isDogStyleId } = styles;
...
const blank = () => ({ defaultStyle: 'classic', breedStyles: {}, revision: 0, updatedAt: null });
...
check('private and prototype-shaped top-level extras are stripped', () => {
  ...
  assert.deepEqual(Object.keys(result).sort(), ['breedStyles', 'defaultStyle', 'revision', 'updatedAt']);
  assert.deepEqual(result, { defaultStyle: 'round', breedStyles: { poodle: 'soft' }, revision: 2, updatedAt: 10 });
  assert.equal(Object.getPrototypeOf(result), Object.prototype); assert.equal(Object.prototype.polluted, undefined);
});
check('parse makes its own map without modifying or retaining the source', () => {
  const source = Object.freeze({ ...blank(), breedStyles: Object.freeze({ poodle: 'mochi' }) });
  const result = parseAppearance(source);
  assert.notEqual(result, source); assert.notEqual(result.breedStyles, source.breedStyles);
...
console.log(`PASS: ${checks} style configuration checks; 16 styles / 7 breeds, backend parity, inheritance, validation, prototype safety, and live wrapper.`);
